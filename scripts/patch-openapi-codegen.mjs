 ...
 * 
 * this also disallows using the default client singleton (unauthenticated)
 *
...
/**
 * Replace the generated default `client` singleton (unauthenticated) with a
 * Proxy that throws on any HTTP call so SDK usage must pass `options.client`.
 * Patched output must stay valid TypeScript after @hey-api/openapi-ts upgrades.
 */
function patchRootClientSingleton(content) {
	const marker = 'export const client = createClient(';
...
function patchFile(filePath) {
	...
	if (path.relative(CLIENT_ROOT, filePath) === 'client.gen.ts') {
		content = patchRootClientSingleton(content);
	}
