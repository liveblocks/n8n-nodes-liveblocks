...
export function buildStructuredOperationProperties(): INodeProperties[] {
	const props: INodeProperties[] = [
		...
		{
			displayName: 'Since (Ms)',
			name: 'q_getFeeds_since',
			type: 'number',
			default: 0,
			description: 'Only return feeds with createdAt after this Unix timestamp in ms; omit or 0 to skip',
			...showOp('getFeeds'),
		},
		{
			displayName: 'Limit',
			name: 'q_getFeeds_limit',
			type: 'number',
			default: 20,
			typeOptions: { minValue: 1, maxValue: 100 },
			description: 'Max feeds to return (1–100, default 20)',
			...showOp('getFeeds'),
		},
		// --- Query: getFeedMessages ---
		{
			displayName: 'Cursor',
			name: 'q_getFeedMessages_cursor',
			type: 'string',
			default: '',
			description: 'Pagination cursor from previous response `nextCursor`',
			...showOp('getFeedMessages'),
		},
		{
			displayName: 'Since (Ms)',
			name: 'q_getFeedMessages_since',
			type: 'number',
			default: 0,
			description: 'Only return messages with createdAt after this Unix timestamp in ms; omit or 0 to skip',
			...showOp('getFeedMessages'),
		},
		{
			displayName: 'Limit',
			name: 'q_getFeedMessages_limit',
			type: 'number',
			default: 20,
			typeOptions: { minValue: 1, maxValue: 100 },
	...
	props.push(
		...
		{
			displayName: 'Feed ID',
			name: 'createFeed_feedId',
			type: 'string',
			default: '',
			required: true,
			description: 'Identifier for the new feed',
			...showOp('createFeed'),
		},
		{
			displayName: 'Metadata',
			name: 'createFeed_metadata',
			type: 'json',
			default: '{}',
			description: 'Optional feed metadata object',
			...showOp('createFeed'),
		},
		{
			displayName: 'Timestamp (ms)',
			name: 'createFeed_timestamp',
			type: 'number',
			default: 0,
			description: 'Optional creation time in Unix ms; omit or 0 for server default',
			...showOp('createFeed'),
		},
		{
			displayName: 'Metadata',
			name: 'updateFeed_metadata',
			type: 'json',
			default: '{}',
			required: true,
			description: 'Feed metadata object to apply',
			...showOp('updateFeed'),
		},
		{
			displayName: 'Message Data',
			name: 'createFeedMessage_data',
			type: 'json',
			default: '{}',
			required: true,
			description: 'JSON object stored as the message payload',
			...showOp('createFeedMessage'),
		},
		{
			displayName: 'Message ID',
			name: 'createFeedMessage_id',
			type: 'string',
			default: '',
		...
		{
			displayName: 'Timestamp (ms)',
			name: 'createFeedMessage_timestamp',
			type: 'number',
			default: 0,
			description: 'Optional creation time in Unix ms; omit or 0 for server default',
			...showOp('createFeedMessage'),
		...
		{
			displayName: 'Message Data',
			name: 'updateFeedMessage_data',
			type: 'json',
			default: '{}',
			required: true,
			description: 'JSON object for the updated message payload',
			...showOp('updateFeedMessage'),
		},
		{
			displayName: 'Timestamp (ms)',
			name: 'updateFeedMessage_timestamp',
			type: 'number',
			default: 0,
			description: 'Optional update time in Unix ms; omit or 0 for server default',
			...showOp('updateFeedMessage'),
		},
	);
